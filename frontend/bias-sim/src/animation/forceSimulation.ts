...
export function applyForceSimulation(
	...
	const simulation = d3
		...
		.force(
			...
			d3
				...
				.id((d: any) => d.id)
				.distance(8 * Math.sqrt(graph.nodes.length))
				.strength(0.1)
		)
		.force('charge', d3.forceManyBody().strength(-1200 * Math.pow(graph.nodes.length, 1 / 4)))
		.force(
			'center',
			d3.forceCenter(width / 2, height / 2).strength(240 * Math.pow(graph.nodes.length, 1 / 4))
		)
	...
	(
		d3.select(nodeGroup).selectAll('.node').data(graph.nodes) as d3.Selection<
			Element,
			Node,
			SVGGElement,
			unknown
...
export function addRepelForceFromNode(
	selectedNode: Node,
