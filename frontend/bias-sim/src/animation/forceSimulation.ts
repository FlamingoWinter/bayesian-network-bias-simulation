...
export function applyForceSimulation(graph: Graph,
	...
	function leftRightForce(alpha: number) {
		const strength = 3 * graph.nodes.length;
	...
	const simulation = d3.forceSimulation(graph.nodes)
		.force('link', d3.forceLink(graph.links).id((d: any) => d.id).distance(15 * graph.nodes.length).strength(0.01 * graph.nodes.length))
		.force('charge', d3.forceManyBody().strength(-360 * graph.nodes.length))
