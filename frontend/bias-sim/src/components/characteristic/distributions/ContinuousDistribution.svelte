<script lang="ts">
	...
	$: xValues = d3.range(
		fullX.domain()[0],
		fullX.domain()[1],
		(fullX.domain()[1] - fullX.domain()[0]) / 100
	);
	$: density = kernelDensityEstimator(
		kernelEpanechnikov,
		xValues
	)(distribution).filter((d) => d.x > minValue && d.x < maxValue);
	$: maxBarY = d3.max(density, (d) => d.y)!;

	let fullX = d3.scaleLinear().domain([0, 0]).range([0, 0]);
...
<g>
	...
	/>
	<path
		stroke={calculateDistributionFill(probabilityType)}
		stroke-width="1.5"
		bind:this={strokePath}
		fill="none"
		d=""
		style="transition: stroke {defaultTransition}"
	/>
	<g>
		<text
			class="bar-label"
	...
	<AxisBottom bind:x {height} {width} {distribution} />
	<AxisBottom bind:x={fullX} extend={true} display={false} {height} {width} {distribution} />
