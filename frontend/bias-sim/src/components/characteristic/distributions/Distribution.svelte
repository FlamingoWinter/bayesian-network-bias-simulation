...
<g>
	{#each bars as bar}
		<g class="bar">
			<rect
				rx="2"
				x={x(bar.category.toString()) ?? 0}
				y={mounted ? y(bar.value) : y(0)}
				width={x.bandwidth()}
				height={mounted ? height - y(bar.value) : 0}
	...
	<AxisLeft {height} display={true} {maxBarY} bind:y />
</g>
