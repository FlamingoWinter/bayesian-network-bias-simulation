<script lang="ts">
	...
	$: absoluteFalseNegativeRateDisparity =
		minAndMaxFalseNegativeRates.maxFalseNegativeRate -
		minAndMaxFalseNegativeRates.minFalseNegativeRate;
	...
	$: absoluteFalsePositiveRateDisparity =
		minAndMaxFalsePositiveRates.maxFalsePositiveRate -
		minAndMaxFalsePositiveRates.minFalsePositiveRate;
	...
	<svelte:fragment slot="content">
		<div class="py-4">
			<p class=" text-center">
				...
				chance of being rejected, while a random competent person from group {minAndMaxFalseNegativeRates.min}
				only has a
				<span class="center block text-2xl font-bold">
			...
			<p class="mt-8 text-center">
				By this metric, there is a <span
					style="color: {levelToColorMapping[falseNegativeBiasLevel]}"
					class="text-lg font-bold">{falseNegativeBiasLevel}</span
				>
				bias against group {minAndMaxFalseNegativeRates.max}
			</p>
		</div>
		...
		<div class="py-4">
			<p class=" text-center">
				...
				chance of being hired mistakenly, while a random person from group {minAndMaxFalsePositiveRates.max}
			...
			<p class="mt-8 text-center">
				By this metric, there is a <span
					style="color: {levelToColorMapping[falsePositiveBiasLevel]}"
					class="text-lg font-bold">{falsePositiveBiasLevel}</span
				>
				bias against group {minAndMaxFalsePositiveRates.min}
			</p>
		</div>
