...
from backend.api.request_types.generate_network_request import (
    NetworkRequest,
        ...
        if raw_request["random_or_predefined"] == "predefined":
            ...
            if predefined_model == "sprinkler":
                ...
                await self.send_and_flush(
                    "Predefined model name was incorrect", error=True
                )
                await self.close(code=1000)
                return
        else:
            request: NetworkRequest = new_random_network_request(**raw_request)

            category_number_distribution = rv_discrete(
                values=(
                    [int(x) for x in request.values_per_variable.keys()],
                    list(request.values_per_variable.values()),
                )
            )

            network: PgmPyNetwork = generate_random_categorical_network(
                int(request.number_of_nodes),
                (int(request.parents_range[0]), int(request.parents_range[1])),
                (
