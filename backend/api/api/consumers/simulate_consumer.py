...
from backend.api.request_types.simulate_request import (
    new_simulate_request,
    SimulateRequest,
)
from backend.api.response_types.bias_response import BiasResponse
...
class SimulateConsumer(GenericConsumer):
    async def receive(self, text_data: Any = None, bytes_data: Any = None) -> None:
        ...
        candidate_group: Applicants = network.sample_applicants(
            request.candidates_to_generate * 2
        )

        await self.send_and_flush(
            f"{request.candidates_to_generate} Candidates Generated"
        )
        ...
        bias_response: BiasResponse = {
            ...
            for [recruiter, bias] in bias_by_recruiter.items()
        }
