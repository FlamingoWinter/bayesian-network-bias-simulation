...
from backend.network.bayesian_network import (
    BayesianNetwork,
    Characteristic,
    num_samples,
)
from backend.network.naming_characteristics.name_characteristics import (
    CharacteristicName,
    affected_characteristics,
    hide_protected_characteristics,
...
class PgmPyNetwork(BayesianNetwork):
    def __init__(
        ...
        self.characteristics: Dict[str, Characteristic] = (
            self.initialise_characteristics_from_model(model)
        )
    ...
    def rename_nodes(self, old_to_new: Dict[str, CharacteristicName]):
        self.renaming = {
            old_node: new_characteristic.name
            for [old_node, new_characteristic] in old_to_new.items()
        }
        ...
        self.application_characteristics = [
            self.renaming[old_ac] for old_ac in self.application_characteristics
        ]
    ...
    def to_network_response(self) -> NetworkResponse:
        ...
        for characteristic_name, characteristic in self.characteristics.items():
            distribution = applicants.characteristic_name_to_distribution(
                characteristic_name
            )
            ...
            characteristic_responses[characteristic.name] = (
    ...
    def sample_conditioned(self):
        ...
        sampled_data = inference.query(
            [
                c
                for c in model_formatted_characteristics
                if c not in self.observed.keys()
            ],
    ...
    def name_characteristics(self, seed=None) -> "PgmPyNetwork":
        ...
        affected_characteristic_choices = random.sample(
            affected_characteristics, len(score_descendants)
        )
        for score_descendant, affected_characteristic in zip(
            score_descendants, affected_characteristic_choices
        ):
        ...
        if hide_protected_characteristics:
            protected_characteristic_choices = [
                CharacteristicName(
                    f"Protected Characteristic {i + 1}", ["1", "2", "3", "4", "5"]
                )
                for i in range(len(nodes_without_ancestors))
            ]
        else:
            protected_characteristic_choices = random.sample(
                protected_characteristics, len(nodes_without_ancestors)
            )

        for node_without_ancestor, protected_characteristic in zip(
            nodes_without_ancestors, protected_characteristic_choices
        ):
        ...
        affector_characteristic_choices = random.sample(
            affector_characteristics, len(direct_predecessors)
        )
        for direct_predecessor, affector_characteristic in zip(
            direct_predecessors, affector_characteristic_choices
        ):
        ...
        direct_children = list(
            set(
                [
                    child
                    for p in nodes_without_ancestors
        ...
        unnamed_nodes = [
            ...
            if node != self.score_characteristic
        ...
        intermediary_characteristic_choices = random.sample(
            intermediary_characteristics, len(nodes_to_be_intermediary)
        )
        for node_to_be_intermediary, intermediary_characteristic in zip(
            nodes_to_be_intermediary, intermediary_characteristic_choices
        ):
        ...
        for old_name, characteristic_name in old_to_new.items():
            characteristic_name.set_number_values(
                len(self.characteristics[old_name].category_names)
            )
