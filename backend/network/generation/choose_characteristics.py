...
def choose_score(graph: nx.DiGraph, more_than_five_descendants=False) -> str:
    ...
    direct_children = list(
        set([child for p in nodes_without_ancestors for child in graph.successors(p)])
    )
    direct_grandchildren = list(
        set([child for p in direct_children for child in graph.successors(p)])
    )

    score_non_candidates = set(
        nodes_without_ancestors + direct_children + direct_grandchildren
    )
    score_candidates = [
        node
        for node in graph.nodes
        if node not in score_non_candidates
...
def choose_application(
    ...
    if condition == 1:
        ...
        candidate_nodes = [
            node
            for node in graph.nodes
            if node != score_characteristic and node != protected_characteristic
        ]
        ...
        return random.sample(candidate_nodes, application_size - 1) + [
        ...
        candidate_nodes = [
            node
            for node in graph.nodes
            if node != score_characteristic
            and node != protected_characteristic
            and node not in proxy_nodes
        ]
        return random.sample(candidate_nodes, application_size // 2) + random.sample(
            list(proxy_nodes), application_size // 2
        )
    elif condition == 4:
        ...
        proxy_nodes = set(
            two_hop
            for one_hop in [protected_characteristic]
            + list(graph.neighbors(protected_characteristic))
