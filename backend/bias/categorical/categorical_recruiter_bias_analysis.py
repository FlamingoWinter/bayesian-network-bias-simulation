...
class CategoricalRecruiterBiasAnalysis:
    def __init__(self, applications: pd.DataFrame, protected_characteristic: Characteristic):
        ...
        self.general = CategoricalGroupPredictionInformation(applications)

        self.by_group: Dict[str, CategoricalGroupPredictionInformation] = {}
        ...
        for group_index, group in enumerate(protected_characteristic.category_names):
            ...
            if len(application_subset) > 0:
                self.by_group[group] = CategoricalGroupPredictionInformation(application_subset)
    ...
    def get_pretty_metric(self, find_metric: Callable[[CategoricalGroupPredictionInformation], float],
