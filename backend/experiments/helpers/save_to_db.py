...
from backend.simulation.measure_bias.recruiter_analysis import RecruiterBiasAnalysis
from backend.utils.entropy import categorical_entropy_of_array
from backend.simulation.build_network.pgmpy_network import PgmPyNetwork


def save_run_to_db(
    engine: Engine,
    network: PgmPyNetwork,
    ...
    proportion_competent_group_1 = ((score == 1) & (protected == 0)).sum() / (
        protected == 0
    ).sum()
    proportion_competent_group_2 = ((score == 1) & (protected == 1)).sum() / (
        protected == 1
    ).sum()
...
def save_recruiter_run_to_db(
    engine: Engine, db_run_id: int, recruiter_bias_analysis: RecruiterBiasAnalysis
) -> str:
    for (
        mitigation_name,
        mitigation_bias_analysis,
    ) in recruiter_bias_analysis.analysis_by_mitigation.items():
        data = pd.DataFrame(
            [
                {
                    "run_id": db_run_id,
                    ...
                    "hired_rate_group_1": mitigation_bias_analysis.by_group[
                        "1"
                    ].hired_rate,
                    "hired_rate_group_2": mitigation_bias_analysis.by_group[
                        "2"
                    ].hired_rate,
                    "fpr": mitigation_bias_analysis.general.false_positive_rate,
                    "fpr_group_1": mitigation_bias_analysis.by_group[
                        "1"
                    ].false_positive_rate,
                    "fpr_group_2": mitigation_bias_analysis.by_group[
                        "2"
                    ].false_positive_rate,
                    "fnr": mitigation_bias_analysis.general.false_negative_rate,
                    "fnr_group_1": mitigation_bias_analysis.by_group[
                        "1"
                    ].false_negative_rate,
                    "fnr_group_2": mitigation_bias_analysis.by_group[
                        "2"
                    ].false_negative_rate,
                    "f_o_r": mitigation_bias_analysis.general.false_omission_rate,
                    "f_o_r_group_1": mitigation_bias_analysis.by_group[
                        "1"
                    ].false_omission_rate,
                    "f_o_r_group_2": mitigation_bias_analysis.by_group[
                        "2"
                    ].false_omission_rate,
                    "fdr": mitigation_bias_analysis.general.false_discovery_rate,
                    "fdr_group_1": mitigation_bias_analysis.by_group[
                        "1"
                    ].false_discovery_rate,
                    "fdr_group_2": mitigation_bias_analysis.by_group[
                        "2"
                    ].false_discovery_rate,
                }
            ]
