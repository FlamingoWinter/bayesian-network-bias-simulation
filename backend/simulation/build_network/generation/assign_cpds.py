...
from backend.utils.entropy import categorical_entropy_of_probabilities


def assign_cpds(
    model,
    graph: nx.DiGraph,
    num_categories_by_node: dict[str, int],
    ...
    for node in graph.nodes:
        ...
        num_categories_for_predecessors = [
            num_categories_by_node[predecessor] for predecessor in predecessors
        ]
        predecessor_values = [
            list(t)
            for t in product(
                *[
                    ...
                    for num_categories in num_categories_for_predecessors
                ]
            )
        ]
        ...
        if len(predecessors) == 0:
            cpd = [generate_categorical_distribution_from_dirichlet(num_categories, 1)]
            node_probabilities_by_node[node] = calculate_node_probabilities(
                predecessors,
            ...
            attempts = 0
            while not (
                mutual_information_range[0]
                <= standardised_mutual_information
                <= mutual_information_range[1]
            ):
                cpd = [
                    generate_categorical_distribution_from_dirichlet(
                        num_categories, alpha
                    )
                    for _ in predecessor_values
                ]
                node_probabilities_by_node[node] = calculate_node_probabilities(
                    predecessors,
                    node_probabilities_by_node,
                ...
                h_y = categorical_entropy_of_probabilities(
                    np.array(list(node_probabilities_by_node[node].values()))
                )
                h_y_given_predecessors = get_entropy_given_predecessors(
                    predecessors, node_probabilities_by_node, cpd, predecessor_values
                )
                mutual_information = h_y - h_y_given_predecessors
                max_mutual_information = min(
                    math.log2(num_categories), math.log2(len(predecessor_values))
                )
...
def calculate_node_probabilities(
    predecessors_of_node: list[str],
    node_probabilities_by_node: dict[str, dict[int, float]],
    ...
    for category in range(num_categories_in_node):
        ...
        if len(predecessors_of_node) == 0:
            ...
            for predecessor_value_index, predecessor_values in enumerate(
                predecessor_values_of_node
            ):
                # cum_prod = p(a) p(b) ...
                cum_prod = 1
                for predecessor, predecessor_value in zip(
                    predecessors_of_node, predecessor_values
                ):
                    cum_prod *= node_probabilities_by_node[predecessor][
                        predecessor_value
                    ]
                probabilities[category] += (
                    cum_prod * node_cpd[predecessor_value_index][category]
                )
    return probabilities


def get_entropy_given_predecessors(
    predecessors_of_node: list[str],
    node_probabilities_by_node: dict[str, dict[int, float]],
    node_cpd: list[ndarray],
    predecessor_values_of_node: list[list],
    ...
    for predecessor_value_index, predecessor_values in enumerate(
        ...
        entropy_given_predecessor_values = categorical_entropy_of_probabilities(
            node_cpd[predecessor_value_index]
        )
...
def generate_categorical_distribution_from_dirichlet(
    num_categories: int, alpha: float = 1
) -> ndarray:
