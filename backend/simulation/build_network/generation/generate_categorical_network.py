...
from backend.simulation.build_network.generation.assign_cpds import assign_cpds
from backend.simulation.build_network.generation.choose_characteristics import (
    choose_score,
    choose_application,
)
from backend.simulation.build_network.generation.generate_dag import generate_random_dag
...
def generate_random_categorical_network(
    ...
    num_categories_by_node: dict[str, int] = {
        node: category_number_dist.rvs() for node in list(graph.nodes)
    }

    try:
        score_characteristic = choose_score(graph)
    except Exception:
        return generate_random_categorical_network(
            nodes,
            parents_range,
    ...
    for node in graph.nodes:
        network.set_category_names_for_characteristic(
            node, [str(x) for x in list(range(1, num_categories_by_node[node] + 1))]
        )
