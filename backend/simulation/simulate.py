...
from backend.simulation.build_network.bayesian_network import Characteristic
...
def simulate(
    ...
    train_candidates, mitigation_candidates, test_candidates = applicants.random_split(
        [0.5, 0.25, 0.25]
    )

    application_train_one_hot = train_candidates.get_applications(
    ...
    for recruiter in recruiters:
        if recruiter.name == "Bayesian Recruiter":
            application_test, application_train, application_holdout = (
                application_test_raw,
                application_train_raw,
                application_mitigation_raw,
            )
        else:
            application_test, application_train, application_holdout = (
                application_test_one_hot,
                application_train_one_hot,
                application_mitigation_one_hot,
            )
        ...
        bias_by_recruiter[recruiter] = RecruiterBiasAnalysis(
            recruiter, test_candidates, application_test, protected_characteristic
        )
